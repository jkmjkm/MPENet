...
from .mpe import MultiPriorEnhancement
from .fouriersr import ResBlock_fre
@ARCH_REGISTRY.register()
class MPENet(nn.Module):
    ...
    def __init__(self,
        ...
        #改进1
        self.mbfse=MultiPriorEnhancement(channels=mid_channels)
        ...
        for i, module in enumerate(modules):
            ...
            self.backbone[module] = ConvResidualBlocks((3 + i) * mid_channels, mid_channels, num_blocks)
            # 改进2
            self.Resfres[module] = ResBlock_fre(n_feats=mid_channels,kernel_size=3)
    ...
    def propagate(self, feats, flows, module_name):
        ...
        for i, idx in enumerate(frame_idx):
            ...
            feat_mpe = self.mbfse(feat_current)
            ...
            if i > 0 and self.is_with_alignment:
                ...
                cond = torch.cat([cond_n1, feat_mpe, cond_n2], dim=1)
                feat_prop = torch.cat([feat_prop, feat_n2], dim=1)
                feat_prop = self.deform_align[module_name](feat_prop, cond, flow_n1, flow_n2)

            # concatenate and residual blocks
            feat = [feat_current] + [feats[k][idx] for k in feats if k not in ['spatial', module_name]] + [feat_mpe,feat_prop]
            ...
            #改进2
            feat_prop = feat_prop + self.Resfres[module_name](self.backbone[module_name](feat))
